...
- Every endpoint wraps its payload in a `getLindsByUniprotResponse` envelope (the `getLinds` spelling is BindingDB's, not a typo) whose records sit under the literal key `bdb.affinities`; reach them with `record_path=getLindsByUniprotResponse.bdb.affinities`.
...
- Prefer these paths: `rest/getLigandsByPDBs`, `rest/getLigandsByUniprot`, `rest/getLigandsByUniprots`, and `rest/getTargetByCompound`.
...
- Common BindingDB patterns:
  ...
  - `{"base_url":"https://bindingdb.org","path":"rest/getTargetByCompound","params":{"smiles":"CC(=O)OC1=CC=CC=C1C(=O)O","cutoff":0.9,"response":"application/json"},"max_items":5,"record_path":"getLindsByUniprotResponse.bdb.affinities"}`
  - `{"base_url":"https://bindingdb.org","path":"rest/getLigandsByUniprot","params":{"uniprot":"P00734","response":"application/json"},"max_items":10,"record_path":"getLindsByUniprotResponse.bdb.affinities"}`
