...
3. **Variant-id format collision in 1000G VCFs.** The 1000G GRCh38 VCFs use `.` (missing) in the ID column rather than `chr:pos:ref:alt`. The on-demand client passes `plink --set-missing-var-ids '@:#:$1:$2'` to rewrite IDs before LD compute (`@` = chromosome, `#` = bp). `$1` / `$2` are **the two alleles in ASCII-sort order, not ref / alt** — plink 1.9 has no REF/ALT naming template ([plink 1.9 data docs](https://www.cog-genomics.org/plink/1.9/data#set_missing_var_ids); use plink2 `--set-all-var-ids` with `$r` / `$a` if you need one). So `1_109270398_G_A` is `1:109270398:A:G` in the panel, and the client sorts the alleles the same way when building `--extract` / `--ld-snp` ids, mapping back to the OT id by lookup. Building them positionally silently loses every partner whose REF sorts after its ALT, because `--extract` ignores absent IDs. Tri-allelic loci that have been split into multiple lines may still produce duplicate IDs; deduplicate the source VCF or `bcftools norm -m -any` upstream if you hit that case.
