...
description: >
  ...
  risk by workstream (Tax, Finance, Legal, HR, IP, Commercial, Regulatory, ESG). Use
  whenever the user is preparing a data room for launch and wants a structured view of
  what might concern a buyer. Do not use for document quality issues like PII or redaction
  (use document-quality-check), or for identifying missing sections (use gap-analysis).
metadata:
  author: Blueflame AI
  version: 1.0.0
  mcp-server: datasite
